...
void
CataCamera::readParameters(const px_comm::CameraInfoConstPtr& cameraInfo)
{
    if (cameraInfo->M.size() != 1 ||
        cameraInfo->D.size() != 4 ||
        cameraInfo->P.size() != 4)
    {
        return;
    ...
    params.xi() = cameraInfo->M.at(0);
    params.k1() = cameraInfo->D.at(0);
    params.k2() = cameraInfo->D.at(1);
    params.p1() = cameraInfo->D.at(2);
    params.p2() = cameraInfo->D.at(3);
    params.gamma1() = cameraInfo->P.at(0);
    params.gamma2() = cameraInfo->P.at(1);
    params.u0() = cameraInfo->P.at(2);
    params.v0() = cameraInfo->P.at(3);
