...
public:
    ...
    template<typename T>
    bool operator() (const T* const q_coeffs, const T* const t_coeffs, T* residuals) const
    {
...
void
HandEyeCalibration::solve(const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d> >& H1,
...
bool
HandEyeCalibration::solveQuadraticEquation(double a, double b, double c, double& x1, double& x2) const
{
...
void
HandEyeCalibration::refine(Eigen::Matrix4d& H_12,
                           const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d> >& H1,
                           const std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d> >& H2) const
{
    ...
    for (size_t i = 0; i < H1.size(); i++)
    {
        ceres::CostFunction* costFunction = 
            new ceres::AutoDiffCostFunction<PoseError, 6, 4, 3>(
                new PoseError(H1.at(i), H2.at(i)));

        problem.AddResidualBlock(costFunction, 0, q.coeffs().data(), t.data()); 
    }
