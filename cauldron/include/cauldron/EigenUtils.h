...
template<typename T>
Eigen::Matrix<T, 4, 4>
invertHomogeneousTransform(const Eigen::Matrix<T, 4, 4>& H)
{
    Eigen::Matrix<T, 4, 4> H_inv = Eigen::Matrix<T, 4, 4>::Identity();

    H_inv.block(0, 0, 3, 3) = H.block(0, 0, 3, 3).transpose();
    H_inv.block(0, 3, 3, 1) = - H_inv.block(0, 0, 3, 3) * H.block(0, 3, 3, 1);
