...
{
public:
    PoseGraphError(const Transform& meas_T_01)
    ...
    PoseGraphError(const Transform& meas_T_01, const std::vector<double>& weight)
     : m_meas_T_01(meas_T_01)
    {
        for (size_t i = 0; i < 6; ++i)
        {
                    ...
                    T* residuals) const
    {
        ...
        Eigen::Matrix<T,4,4> pred_H_01 = H1 * invertHomogeneousTransform(H0);

        Eigen::Matrix<T,4,4> err_H = meas_H_01 * invertHomogeneousTransform(pred_H_01);
